        ...
        if not isinstance(nwt, (int, float)):
            try:
                ...
                raise TypeError("New weight must be numeric.")
                    ...
                    # https://stackoverflow.com/questions/23306364/retrieving-loc-index-values-in-pandas-dataframe
#        self._gamestats.reset_index(inplace=True) # resetting the index to access faces
#        self._gamestats.replace(to_replace=self._gamestats.loc['self.face', 'faces'], value=self.nwt, inplace=True) # !! 
#        self._gamestats.set_index(self._gamestats.loc['faces'], inplace= True) # reestablishing faces as the index
        
                    # https://stackoverflow.com/questions/38980507/moving-desired-row-to-the-top-of-pandas-data-frame/38988241
#        appear_first = [self.face]
#        new_index_order = [*appear_first, *self._gamestats.index.difference(appear_first)]
#        self._gamestats.loc[
#        self._gamestats.loc[new_index_order].replace(self._gamestats.loc[:], self.nwt, inplace=True)
